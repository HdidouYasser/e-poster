...
import {
  ...
  Activity, Monitor, Presentation, BarChart3, Download, UserCog
} from "lucide-react";
...
// Pages accessible to Event Managers only (adminOnly: false)
      ...
      <aside className="w-64 border-r border-zinc-200/60 bg-white flex flex-col shrink-0">
        ...
        <div className="p-3 border-t border-zinc-100 space-y-1">
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 bg-zinc-50 rounded-xl border border-zinc-100">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-bold text-white uppercase shrink-0 shadow-sm ${isManager ? "bg-blue-600" : "bg-zinc-900"}`}>
              {username ? username.substring(0, 2) : "AD"}
            </div>
            <div className="overflow-hidden flex-1 min-w-0">
              <div className="text-xs font-semibold text-zinc-900 truncate">{username || "Admin"}</div>
              <div className={`text-[10px] font-medium ${isManager ? "text-blue-500" : "text-zinc-400"}`}>{roleLabel}</div>
            </div>
          </div>
        </div>
      </aside>
